...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#plan'
            ,height: 312
            ,url: '/admin/getrecharge' //数据接口
            ,page: true //开启分页
            ,cols: [[ //表头
                {field: 'name', title: '用户', width:100}
                ,{field: 'transfer_name', title: '汇款名称'}
                ,{field: 'transfer_no', title: '汇款账号'}
                ,{field: 'quantity', title: '金额', sort: true}
                ,{field: 'status', title: '状态', templet: '#closeTpl', width:75}
                ,{fixed: 'right', title: '操作',width:130, align:'center', toolbar: '#bar'}
            ]]
        });
        table.on('tool(plan)', function(obj){
            var data = obj.data;
            if(obj.event === 'edit'){
                $.post('/admin/addbalance/'+data.id,{'_token':'{{csrf_token()}}'},function (dd) {
                    if(dd.code == 0){
                        layer.msg(dd.message,{icon:1});
                        table.reload('plan', {
                            url: '/admin/getrecharge'
                        });
                    }
                });
            } else if(obj.event === 'del'){
                layer.confirm('删除操作不可逆，是否继续？', function(index){
                    $.post('/admin/delbalance/'+data.id,{'_token':'{{csrf_token()}}'},function (dd) {
                        if(dd.code == 0){
                            obj.del();
                            layer.msg(dd.message,{icon:1});
                            layer.close(index);
