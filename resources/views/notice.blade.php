                                        ...
                                        <div class="table-wrap">
                                            <div class="table-responsive">
                                                <table class="table">
                                                    ...
                                                    <tr>
                                                        <td class="">{{date('Y-m-d H:i:s',$no->time)}}</td>
                                                        <td class="">
                                                            @if($no->type==1)
                                                                应用{{$no->filename}}已提交到后台，我们会在10分钟以内为您处理，点击
                                                                <a href="/sign/lists">{{env('APP_URL')}}/sign/lists</a>查看详情。
                                                                @elseif($no->type == 2)
                                                                应用{{$no->filename}}签名完成，点击
                                                                <a href="/sign/lists">{{env('APP_URL')}}/sign/lists</a>查看详情。
                                                                @elseif($no->type == 3)
                                                                充值已到账，点击
                                                                <a href="/user/order">{{env('APP_URL')}}/user/order</a>查看详情。
                                                                @elseif($no->type == 4)
                                                                签名续费成功，点击
                                                                <a href="/apps">{{env('APP_URL')}}/apps</a>查看详情。
                                                            @endif
                                                        </td>
                                                    </tr>
                                                        @endforeach
