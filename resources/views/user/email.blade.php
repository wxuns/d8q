                                    ...
                                    <div class="row">
                                        <div class="col-sm-7">
                                            <div class="form-group clearfix current-email">
                                                <label class="fl">当前邮箱</label>
                                                <span class="fl" id="nick">{{$user}}</span>
                                            </div>
                                            <div class="form-group">
                                                <label>新邮箱</label>
                                                <div class="clearfix">
                                                    <input type="text" id="email" name="email" class="text form-control" placeholder="请输入您要更换的邮箱账号">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label>新邮箱验证码</label>
                                                <div class="clearfix row">
                                                    <div class="col-sm-9">
                                                        <input type="text" class="form-control" placeholder="请输入新邮箱的验证码" name="code">
                                                    </div>
                                                    <div class="col-sm-3">
                                                        <button type="button" class="ms-btn-default ms-btn w-fluid  send-email-code" id="sendcode">
                                                            获取验证码
                                                        </button>
                                                    </div>
                                                </div>
...
<script>
    ...
    $('#sendcode').click(function () {
        if (patt.test(email.val())){
            $.post('sendemail',{email:email.val(),_token:'{{csrf_token()}}'},function(data){
                second = 60;
                var time = setInterval(function () {
                    if (second>0){
                        $('#sendcode').html(--second + ' s');
    ...
    $('#submitButton').click(function () {
        if (patt.test(email.val())&&code.val()){
            $.post('',{
                code:code.val()
                ,email:email.val()
                ,_token:'{{csrf_token()}}'
